...
def add_ts(tablename, ins=True, upd=True):
    if ins:
        op.execute('ALTER TABLE "{}" ADD inserted '
                   "TIMESTAMP WITH TIME ZONE DEFAULT NOW()".format(tablename))
    if upd:
        op.execute(('ALTER TABLE "{}" ADD updated '
...
def upgrade():
    ...
    op.add_column('user', Column('active', Boolean, nullable=False,
                                 server_default='true'))
    op.add_column('user', Column('confirmed_at', DateTime(timezone=True)))
    op.add_column('team', Column('owner', Unicode(), ForeignKey(
    ...
    op.create_table(
        ...
        Column('user', Unicode, ForeignKey('user.name',
                                           ondelete='CASCADE',
                                           onupdate='CASCADE')),
    ...
    op.create_table(
        ...
        Column('questionnaire', Unicode,
               ForeignKey('questionnaire.name',
    ...
    op.execute('ALTER TABLE team ADD CONSTRAINT team_confirmation_key '
