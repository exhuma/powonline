...
def build_docker(ctx, environment="staging"):  # type: ignore
    ...
    with Connection(host) as conn:
        pytmp = conn.run(
            "mktemp -d /tmp/deploy-powonline-be-XXXX"
        ).stdout.strip()
        try:
        ...
        dbtmp = conn.run(
            "mktemp -d /tmp/deploy-powonline-db-XXXX"
        ).stdout.strip()
        try:
...
def deploy(ctx, environment="staging"):  # type: ignore
    ...
    host = ROLEDEFS[environment]
    with Connection(host) as conn:
        _safe_put(conn, "api.env", "%s/api.env" % DEPLOY_DIR)
        _safe_put(
            conn, "docker-compose.yaml", "%s/docker-compose.yaml" % DEPLOY_DIR
        )
