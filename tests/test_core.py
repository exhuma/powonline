...
async def test_get_assignments(dbsession: AsyncSession, seed):
...
async def test_questionnaire_scores(dbsession: AsyncSession, seed, test_config):
    ...
    result = await core.questionnaire_scores(test_config, dbsession)
    expected = {
        "team-red": {
...
async def test_set_questionnaire_score(
    dbsession: AsyncSession, test_config, seed
):
    test_config.read_string(
        dedent(
            """\
    ...
    new_data = await core.questionnaire_scores(test_config, dbsession)
    expected = {
        "team-red": {
