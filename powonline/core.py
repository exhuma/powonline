...
async def set_questionnaire_score(
    config, session: AsyncSession, team, station, score
):
    mapping = {}
    ...
    query = select(model.Questionnaire).filter(
        model.Questionnaire.name.ilike(questionnaire_name)  # type: ignore
    )
    result = await session.execute(query)
    existing_questionnaire = result.scalar_one_or_none()
